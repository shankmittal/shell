...
_adb() {
    ...
    OPTIONS="-d -e -s -p"
    COMMAND="devices connect disconnect push pull sync shell emu logcat lolcat forward jdwp install uninstall bugreport help version start-server kill-server get-state get-serialno status-window remount reboot reboot-bootloader root usb tcpip wait-for-device"
    ...
    case $where in
        OPTIONS|OPT_SERIAL|OPT_PATH)
            ...
            ;;
        OPT_SERIAL_ARG)
            local devices=$(command adb devices 2> /dev/null | awk '{ print $1 }')
