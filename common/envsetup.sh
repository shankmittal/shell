...
function reposynconly() {
    ...
    if [[ -z "$1" ]]; then
        # repo sync  -j$MAKE_THREAD --no-tags -c && repo forall -j$MAKE_THREAD -c 'test -e .lfsconfig && git lfs pull'
        repo sync  -j$MAKE_THREAD --no-tags -c -q -d
        return
    fi
...
function reposync() {
    ...
    RPATH=$(repo forall $PROJECT -c 'echo $REPO_PATH')

    repo sync -j$MAKE_THREAD --no-tags -c $PROJECT
    if [ "$RPATH" -eq '' ]; then
        RPATH="$PROJECT"
        repolfs "$RPATH"
        return
...
function repolfs() {
    ...
    if [ ! -z $PROJECT ]; then
        ( cd $PROJECT && test -e .lfsconfig && git lfs pull )
        return
    fi

    repo forall -j$MAKE_THREAD  -c 'test -e .lfsconfig && echo $REPO_PATH && git lfs pull'
}
...
function refix()
{
	...
	if [ ! -f $HOME/re-$1 ]; then
		echo "No $HOME/re-$1 found!"
		return -1
	fi

	rm $HOME/re 2> /dev/null
	ln -s $HOME/re-$1 $HOME/re
}
